...
{
public:
  Process_2DGrid(int _P, int _Q, MPI_Comm _parent_comm) : the_P(_P), the_Q(_Q),
						      the_parent_comm(_parent_comm)
               {MPI_Comm_size(_parent_comm,&parent_size);
                MPI_Comm_rank(_parent_comm,&myrank_in_parent_comm);
		pq_of_rank(myrank_in_parent_comm, the_p, the_q);
  ...
  virtual ~Process_2DGrid() {MPI_Comm_free(&the_row_comm); MPI_Comm_free(&the_col_comm);}
  ...
  int  rank_of_pq(int _p, int _q) const {return _p*the_Q+_q;}
  void pq_of_rank(int R, int &_p, int &_q) const {_p=R/the_Q; _q=R%the_Q;}
...
Kokkos::initialize( argc, argv );
  ...
  if(0 == myrank)
  {
     ...
     array[3] = Q;
     array[4] = iterations;
     printf("Broadcasting M=%d, N=%d, P=%d, Q=%d, iterations=%d\n",M,N,P,Q,iterations);
  }
...
void parallel_code(int M, int N, int iterations, Process_2DGrid &grid)
{
  ...
  if(grid.q()==0)
  {
     MPI_Type_vector(m, 1, N, MPI_CHAR, &vertical_slice_in);
  ...
  MPI_Type_vector(m, 1, (n+2*1), MPI_CHAR, &vertical_slice_out);
...
void update_domain(Domain &new_domain, Domain &old_domain, Process_2DGrid &grid)
{
  ...
  Kokkos::View<char*> left_col("left_col",m);
  Kokkos::View<char*> west_halo("west_halo",n);
  Kokkos::View<char*> right_col("right_col",m);
  Kokkos::View<char*> east_halo("east_halo",n);
  ...
  Kokkos::parallel_for("fill_left",m,KOKKOS_LAMBDA(const int i) {
		left_col[i] = old_domain(i,0);		  
	});
  Kokkos::Fence();
  MPI_Isend(left_col, m, MPI_CHAR, West_qrank, EAST_HALO, grid.row_comm(), &request[10]);

  Kokkos::parallel_for("fill_right",n,KOKKOS_LAMBDA(const int i) {   // fill right
  ...
  Kokkos::parallel_for("halo_space",m,KOKKOS_LAMBDA(const int i)
  { 
  ...
  Kokkos::parallel_for("row",m,KOKKOS_LAMBDA(const int i)
	{
	Kokkos::parallel_for("col",n,KOKKOS_LAMBDA(const int j)
	{
      int neighbor_count =
